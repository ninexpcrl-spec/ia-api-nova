...
<body>
    <div class="container">
        ...
        <div class="subtitle">Sistema inteligente de análise de Job IDs</div>
        ...
        async function carregarJobsRuins() {
            ...
            const html = blacklist.map(job => \`
                <tr>
                    <td><code>\${job.job_id?.substring(0, 30)}...</code></td>
                    <td>\${job.brainrots?.length || 0}</td>
                    <td>\${job.tentativas} tentativas sem sucesso</td>
                    <td><button class="btn btn-danger" onclick="removerJob('\${job.job_id}')">🗑️ Remover</button></td>
                </tr>
            \`).join('');
            document.getElementById('jobs-ruins-list').innerHTML = html || '<tr><td colspan="4">Nenhuma Job ID na blacklist</td></tr>';
        }
